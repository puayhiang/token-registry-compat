...
import { TitleEscrowVersion, TokenRegistryVersion } from '../src/contracts/utils';
import {deployTokenRegistry} from './utils'

chai.use(solidity);

describe("TitleEscrowCloneable", () => {
  ...
  // "Initialising contract factories and accounts for TitleEscrow tests",
  beforeAll(async () => {
    [carrier1, beneficiary1, beneficiary2, holder1, holder2] = new MockProvider().getWallets();
...
// "Initialising fresh Token Registry for each test",
  beforeEach(async () => {
    ERC721Instance = deployTokenRegistry(carrier1);
 ...
 export type DeployTitleEscrowType = {
  ...
  const makeTitleEscrow = async (beneficiary, holder, registry = ERC721Address) => {
    // const titleEscrowFactory = new TitleEscrowFactory(carrier1);
    // const titleEscrowInfo = {
    //   'tokenRegistry': registry,
    //   'beneficiary': beneficiary,
    //   'holder': holder,
      // titleEscrowFactoryAddress: string,
      // 'version': TitleEscrowVersion.V3,
      // overrides?: Overrides,
    // }
    const titleEscrowTx = await ERC721Instance.mintTitle(beneficiary,holder,SAMPLE_TOKEN_ID);
    const tx = await titleEscrowTx.wait();
    const teLog = tx.events.find((e) => e.event === "TitleEscrowDeployed");
    const teAddr = teLog.args.escrowAddress;
    return TitleEscrowFactory.connect(teAddr, carrier1);
  };
  ...
  it("should have the correct ERC165 interface support", async () => {
    ...
    const ITitleEscrowInterfaceId = "0xdcce2211";
